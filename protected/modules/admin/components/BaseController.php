...
class BaseController extends BController{
	...
	public function checkflag($action, $controller){
		...
		if (user()->id==1)
			return true;
		if (in_array(strtolower(trim($moduleId.'/'.$controller.'/'.$action, '/')), $this->access))
			return true;
		if (in_array(strtolower(trim($moduleId.'/'.$controller.'/*', '/')), $this->access))
			return true;
		...
		/*dump(strtolower(trim($moduleId.'/'.$controller.'/'.$action, '/')));
		dump($rules);
		exit;*/
