...
class CHtml
{
	...
	{
		$htmlOptions['name']=$name;
		if($htmlOptions['theme']!='none'){
		$htmlOptions['class']=$htmlOptions['class'].' chosen-select';
		}
		$htmlOptions['data-placeholder']="请选择";
		$htmlOptions['style']=$htmlOptions['style'].' position: relative;';
		if(!isset($htmlOptions['id']))
			$htmlOptions['id']=self::getIdByName($name);
		elseif($htmlOptions['id']===false)
			unset($htmlOptions['id']);
	...
	static function switchButton($name,$select,$data=array('1'=>'YES','0'=>'NO'),$ajaxUrl=''){
		...
		$off='switch-off';
		if($select==$keys[0])$off='';
		$html=<<<EOT
		<div class="switch switch-small has-switch" data-left={$keys[0]} data-right={$keys[1]}  data-url={$ajaxUrl}>
			<div class="switch-on switch-animate {$off}" >
			     <input type="hidden"  name="{$name}" value="{$select}">
				 <span class="switch-left switch-small">{$value[0]}</span>
				 <label class="switch-small">&nbsp;</label>
				 <span class="switch-right switch-small">{$value[1]}</span>
			</div>
		</div>
EOT;
		return $html;
	}
	static function groupButton($name,$select,$data,$ajaxUrl=''){
		//$data=array('0'=>'NO','1'=>'YES','3'=>'SSS');
		$keys=array_keys($data);
		$value=array_values($data);
		foreach ($data as $kk=>$val){
			...
			$option.="<option value='$kk' $checked>$val</option>";
			$buttons.='<button type="button" class="gbtn gbtn-default '.$on.'" data="'.$kk.'" selectid="'.$name.'"><i class="glyph-icon '.$icon.'"></i>'.$val.'</button>';
			unset($on);unset($checked);unset($icon);
		}
		$html=<<<EOT
		<div class="btn-group">
...
EOT;
	...
	static function multipleListButton($name,$select,$data,$ajaxUrl=''){
		//$data=array('0'=>'NO','1'=>'YES','3'=>'SSS');
		$keys=array_keys($data);
		$value=array_values($data);
		foreach ($data as $kk=>$val){
			if(in_array($kk, (array)$select)){
				$checked='selected';
				$on='primary-bg';
				$icon='icon-ok';
			...
			$option.="<option value='$kk' $checked>$val</option>";
			$buttons.='<a href="javascript:;" class="btn medium ui-state-default '.$on.'" data="'.$kk.'">
			            <span class="button-content">
			                <i class="glyph-icon  float-left '.$icon.'"></i>
			               '.$val.'
			            </span>
			        </a>&nbsp;';
		...
		<div class="multiple-select">
			<select name="{$name}[]" style="display:none" class="multipleList" multiple="multiple">
				{$option}
			</select>
			{$buttons}
		</div>
