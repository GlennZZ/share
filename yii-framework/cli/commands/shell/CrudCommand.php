...
EOD;
	...
	public function generateActiveField($modelClass,$column)
	{
		...
		else
		{
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}',,array('class'=>'col-md-6 float-left','placeholder'=>'{$column->name}'))";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength,'class'=>'col-md-6 float-left','placeholder'=>'{$column->name}'))";
			}
		}
	}
