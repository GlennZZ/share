...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveField($modelClass,$column,$htmlOptions='',$check=true)
	{	if(!$htmlOptions)$htmlOptions="'class'=>'col-md-6 float-left'";
		if($column->type==='boolean')
		...
		else
		{
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}',array(\$htmlOptions))";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				if($check){
				return "\$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength,$htmlOptions))";
				}else{
					return "\$form->{$inputField}(\$model,'{$column->name}',array(".$htmlOptions."))";
				}
			}
